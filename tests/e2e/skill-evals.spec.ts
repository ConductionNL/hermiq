...
async function openSeededDataset(page: Page): Promise<void> {
	...
	await expect(page.getByText('woo-triage-paired-eval').first()).toBeVisible({
		timeout: 30_000,
	})
...
test.describe('skill-scoped paired evals (skill-evals)', () => {
	...
	test('dataset detail links and unlinks a skill through the skill panel', async ({
		page,
	}) => {
		...
		await page
			.getByRole('option')
			.filter({ hasText: 'tender-summary' })
			.first()
			.click()
		await page.getByRole('button', { name: 'Link', exact: true }).click()
		await expect(page.getByText('Skill linked.').first()).toBeVisible({
			timeout: 15_000,
		})
		await expect(page.getByText('tender-summary').first()).toBeVisible()

		const row = page
			.locator('.skill-link-panel__item', { hasText: 'tender-summary' })
			.first()
		await row.getByRole('button', { name: 'Unlink skill' }).click()
		await expect(page.getByText('Skill unlinked.').first()).toBeVisible({
			timeout: 15_000,
		})
	})
	...
	test('paired baseline toggle is gated on linked skills and states the cost', async ({
		page,
	}) => {
		await openSeededDataset(page)
		...
		const toggle = page.getByRole('checkbox', {
			name: 'Paired baseline (with vs without skills)',
		})
	...
	test('SkillDetail evidence card shows an honest empty state before any run', async ({
		page,
	}) => {
		...
		await page
			.locator('tr', { hasText: 'woo-request-triage' })
			.first()
			.getByText('woo-request-triage')
			.first()
			.click()

		await expect(page.getByText('Eval evidence').first()).toBeVisible({
			timeout: 30_000,
		})
	...
	test('agent detail surfaces the evalBaselineMode description as an info affordance', async ({
		page,
	}) => {
		...
		if ((await firstRow.count()) === 0) {
			...
			await expect(page.getByText('e2e-baseline-agent').first()).toBeVisible({
				timeout: 30_000,
			})
		}
		...
		await expect(page.getByText('Eval baseline mode').first()).toBeVisible({
			timeout: 30_000,
		})
		await page
			.getByRole('button', {
				name: 'What does the eval baseline mode change?',
			})
			.click()
		// The popover renders the register property's description: semantics + cost.
		await expect(
			page
				.getByText(/JOINT contribution of the whole linked skill set/)
				.first(),
		).toBeVisible()
		await expect(
			page.getByText(/\(N\+1\)x the token cost/).first(),
		).toBeVisible()
	})
	...
	test('a live paired run proves the seam and refreshes the evidence card', async ({
		page,
	}) => {
		...
		await page
			.locator('.eval-run-panel-widget__agent-picker [role="option"]')
			.first()
			.click()
		await page
			.getByText('Paired baseline (with vs without skills)')
			.first()
			.click()
		await page.getByRole('button', { name: 'Run', exact: true }).click()
		await expect(page.getByText(/Eval run complete/).first()).toBeVisible({
			timeout: 300_000,
		})
		...
		await page
			...
			.first()
			.click()
		await expect(page.getByText('Last validated').first()).toBeVisible({
			timeout: 30_000,
		})
