...
test.describe('talk-agent-sessions — a session owns its Talk room', () => {
	...
	test('a session for a Talk-enabled agent is created owning a room named after it', async ({
		page,
	}) => {
		const token = await harvestToken(page)
		...
		expect(
			...
			'a Talk-enabled agent must get a room',
		).toBeTruthy()
		expect(
			session.talkRoomOrigin,
			'the room must be recorded as hermiq-created',
		).toBe('created')
	...
	test('renaming the session renames the room it owns', async ({ page }) => {
		...
		expect(
			roomToken,
			'Talk is enabled, so a Talk-enabled agent must own a room',
		).not.toEqual('')
	...
	test('a session for a Talk-DISABLED agent gets no room, and is still usable', async ({
		page,
	}) => {
		const token = await harvestToken(page)
		await resolveRegisterSchema(page.request, token, 'agent')
		...
		expect(
			created.ok(),
			'a session must be created with or without Talk',
		).toBeTruthy()
		...
		expect(
			session.talkRoomToken ?? '',
			'a Talk-disabled agent must get no room',
		).toEqual('')
		// Absent origin reads as `bound`, which keeps the mention gate on — the
		// safe default, and the reason no backfill was needed.
		expect(session.talkRoomOrigin ?? '', 'origin must stay unset').not.toBe(
			'created',
		)
	})

	test('the hermiq chat surface still renders with the session-room wiring in place', async ({
		page,
	}) => {
