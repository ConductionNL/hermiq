...
test.describe('hermiq agents + approvals', () => {
	...
	test.fixme('Agents index renders: page identity, resolved route, Add button, rows or empty state', async ({ page }) => {
	...
	test.fixme('Agent detail renders the seeded agent (API-seeded via OpenRegister)', async ({ page }) => {
	...
	test.fixme('Agent detail for a nonexistent id shows a graceful state, not a crash', async ({ page }) => {
	...
	test.fixme('Approvals inbox renders: heading + table or empty state, no error card', async ({ page }) => {
