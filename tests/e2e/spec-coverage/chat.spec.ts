...
test.describe('hermiq chat surface (UI mechanics, no LLM required)', () => {
	...
	test.fixme('chat page renders: conversation list column + thread empty state, composer absent without a conversation', async ({ page }) => {
	...
	test.fixme('with a seeded agent: start conversation, Send disabled/enabled coherent with input, optimistic bubble + honest turn outcome', async ({ page }) => {
