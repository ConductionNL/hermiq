...
namespace OCA\Hermiq\Tests\Unit;

use OCA\Hermiq\Repair\RenameDutchColumns;
...
/**
 * @covers \OCA\Hermiq\Repair\RenameDutchColumns
 */
class RenameDutchColumnsTest extends TestCase {
	...
	public function testGetNameDescribesTheMigration(): void {
		$step = $this->step([], [], []);
		$this->assertStringContainsString('hermiq', strtolower($step->getName()));
	...
	public function testRenamesWhenOnlyTheDutchColumnExists(): void {
		...
		$step = $this->step([7], [$table], [$table => ['id', 'organisatie']]);
		$step->run($this->repairOutput());
		$this->assertCount(1, $this->executed);
		$this->assertStringContainsString('RENAME COLUMN "organisatie" TO "organisation"', $this->executed[0]);
	...
	public function testAllMappedColumnsInATableAreMigrated(): void {
		...
		$step = $this->step(
			[7],
			[$table],
			[$table => ['id', 'organisatie', 'referentie', 'soort', 'wettelijke_grondslag']]
		);
		$step->run($this->repairOutput());
		$this->assertCount(4, $this->executed);
		$this->assertStringContainsString('"wettelijke_grondslag" TO "statutory_basis"', implode(' ', $this->executed));
	...
	public function testBackfillsWhenBothColumnsExist(): void {
		$table = 'oc_openregister_table_7_3';
		$step = $this->step([7], [$table], [$table => ['id', 'organisatie', 'organisation']]);
		$step->run($this->repairOutput());
		$this->assertCount(1, $this->executed);
		$this->assertStringContainsString('UPDATE', $this->executed[0]);
		$this->assertStringContainsString('"organisation" = "organisatie"', $this->executed[0]);
		$this->assertStringContainsString('"organisation" IS NULL', $this->executed[0]);
	...
	public function testTableWithoutTheDutchColumnIsUntouched(): void {
		$table = 'oc_openregister_table_7_3';
		$step = $this->step([7], [$table], [$table => ['id', 'organisation']]);
		$step->run($this->repairOutput());
		$this->assertSame([], $this->executed);
	...
	public function testNonNumericSuffixIsNotAShard(): void {
		...
		$step = $this->step([7], [$table], [$table => ['organisatie']]);
		$step->run($this->repairOutput());
		$this->assertSame([], $this->executed);
	...
	public function testFailedStatementIsLoggedAndDoesNotThrow(): void {
		$table = 'oc_openregister_table_7_3';
		$step = $this->step([7], [$table], [$table => ['organisatie']], failOn: 'RENAME COLUMN');
		$step->run($this->repairOutput());
		$this->assertSame([], $this->executed);
	...
	public function testAllShardsOfTheRegisterAreMigrated(): void {
		...
		$step = $this->step([7], [$a, $b], [$a => ['organisatie'], $b => ['organisatie']]);
		$step->run($this->repairOutput());
	...
	public function testSummaryReportsCounts(): void {
		...
		$step = $this->step(
			[7],
			[$a, $b],
			[$a => ['organisatie'], $b => ['organisatie', 'organisation']]
		);
		$step->run($this->repairOutput());
