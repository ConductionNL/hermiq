...
function readConfig(array $argv): array
{
    ...
    foreach (array_slice($argv, 1) as $arg) {
        if ($arg === '--gate-check') {
            $cfg['gateCheck'] = true;
        } else if (str_starts_with($arg, '--prompt-set=') === true) {
            $cfg['promptSet'] = substr($arg, strlen('--prompt-set='));
        } else if (str_starts_with($arg, '--prompt=') === true) {
            $cfg['prompt'] = substr($arg, strlen('--prompt='));
        } else if (str_starts_with($arg, '--schedule-id=') === true) {
            $cfg['scheduleId'] = substr($arg, strlen('--schedule-id='));
        } else if (str_starts_with($arg, '--organisation=') === true) {
            $cfg['organisation'] = substr($arg, strlen('--organisation='));
        } else if (str_starts_with($arg, '--engine-flag-state=') === true) {
...
function parseSse(string $body): array
{
    ...
    foreach (preg_split('/\n\n+/', str_replace("\r\n", "\n", $body)) as $block) {
        ...
        foreach (explode("\n", $block) as $line) {
            ...
            } else if (str_starts_with($line, 'data:') === true) {
