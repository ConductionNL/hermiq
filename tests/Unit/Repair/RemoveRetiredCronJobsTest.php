...
use OCA\Hermiq\Repair\RemoveRetiredCronJobs;
...
/**
 * @covers \OCA\Hermiq\Repair\RemoveRetiredCronJobs
 *
 * @spec exclude No canonical spec covers the OCA\Hermiq\Cron ->
 *  OCA\Hermiq\BackgroundJob move; ADR-100 Decision 3 is an architecture record,
...
final class RemoveRetiredCronJobsTest extends TestCase {
	...
	public function testRemovesTheRegisteredRetiredClass(): void {
		...
		$this->assertContains('OCA\\Hermiq\\Cron\\ScheduleTask', $removed);
	...
	public function testAFailureOnOneClassDoesNotStopTheRest(): void {
		...
		$this->assertContains('OCA\\Hermiq\\Cron\\WebhookAgentRunJob', $attempted, 'every class must be attempted even when each one throws');
