...
function withStub(fn) {
    ...
    return Promise.resolve(fn({ providers, runner, log }))
        .finally(() => {
            if (prevBin === undefined) { delete process.env.RUNNER_ANTHROPIC_BIN; } else { process.env.RUNNER_ANTHROPIC_BIN = prevBin; }
            if (prevLog === undefined) { delete process.env.RUNNER_MCP_TEST_LOG; } else { process.env.RUNNER_MCP_TEST_LOG = prevLog; }
            if (prevPassthrough === undefined) { delete process.env.RUNNER_PASSTHROUGH_ENV; } else { process.env.RUNNER_PASSTHROUGH_ENV = prevPassthrough; }
...
const MCP_CONFIG = {
    mcpServers: {
        hermiq: {
            ...
            headers: { Authorization: `Bearer ${RUN_TOKEN}`, 'OCS-APIRequest': 'true' },
        },
    },
};
...
test('governed anthropic argv contains the exact lockdown flags', () => {
    ...
    assert.strictEqual(args[cfgIdx + 1], '/scratch/mcp.json', '--mcp-config carries the path');
    assert.ok(args.includes('mcp__hermiq__*'), '--allowedTools admits only Hermiq MCP tools');
});
...
test('text-only anthropic argv carries NONE of the governed flags', () => {
    ...
    assert.ok(!args.includes('--strict-mcp-config'), 'no --strict-mcp-config on a text-only turn');
...
test('run() writes a 0600 MCP file, keeps the token off argv, and cleans up', () => {
    return withStub(async ({ providers, runner, log }) => {
        ...
        assert.ok(logged.includes(RUN_TOKEN), 'run token IS present inside the 0600 config file');

        // Scratch (and the mcp.json inside it) is removed by cleanup() after close.
        const contentLine = logged.split('\n').find((l) => l.startsWith('MCP_CONTENT:'));
...
test('run() refuses to spawn a governed turn whose provider drops --strict-mcp-config', () => {
    return withStub(async ({ providers, runner }) => {
        ...
        provider.args = (model, options) => ['-p', '--output-format', 'json', '--mcp-config', options.mcpConfigPath];
