...
function startFakePdp(handler) {
    ...
    const server = http.createServer((req, res) => {
        ...
        req.on('end', () => {
            calls.push({ headers: req.headers, body: raw === '' ? {} : JSON.parse(raw) });
            handler(req, res, raw);
        });
    });
...
function listen(mod) {
    return new Promise((resolve) => {
        mod.server.listen(0, '127.0.0.1', () => {
            resolve({ port: mod.server.address().port, close: () => mod.server.close() });
        });
    });
}
...
function connectThrough(port, target, token) {
    return new Promise((resolve, reject) => {
        const headers = {};
        if (token !== null) {
            headers['Proxy-Authorization'] = `Basic ${Buffer.from(`run:${token}`).toString('base64')}`;
        }
...
test('a PDP verdict of allowed:true opens the tunnel', async () => {
    ...
    const out = await connectThrough(proxy.port, `127.0.0.1:${upstreamPort}`, 'tok-123');
...
test('a PDP verdict of allowed:false denies (the non-allowlisted host case)', async () => {
    const pdp = await startFakePdp((req, res) => {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ allowed: false, code: 'host_not_allowlisted', message: 'nope' }));
    });
    const proxy = await listen(loadProxy({ EGRESS_PDP_URL: pdp.url }));
...
test('the backstop does not depend on the CLI flags — any client is denied the same', async () => {
    ...
    const out = await connectThrough(proxy.port, 'telemetry.example.com:443', 'tok-123');
    assert.strictEqual(out.status, 403, 'the network layer denies regardless of how the CLI was invoked');
...
test('an UNREACHABLE PDP denies (fail-closed)', async () => {
    ...
    const proxy = await listen(loadProxy({ EGRESS_PDP_URL: 'http://127.0.0.1:1/authorize' }));
...
test('a TIMING-OUT PDP denies (fail-closed)', async () => {
    ...
    const proxy = await listen(loadProxy({ EGRESS_PDP_URL: pdp.url, EGRESS_PDP_TIMEOUT_MS: '150' }));
...
test('a TRUTHY-but-not-true allowed value denies (only a literal true permits)', async () => {
    ...
    const out = await connectThrough(proxy.port, 'api.anthropic.com:443', 'tok-123');
    assert.strictEqual(out.status, 403, '"yes" is truthy in JS — it must NOT be read as a permit');
...
test('the run token is forwarded as a bearer token and the EXACT host:port is asked about', async () => {
    ...
    assert.deepStrictEqual(pdp.calls[0].body, { host: 'api.anthropic.com', port: 443 });
...
test('tokenFromProxyAuth keeps a token containing colons intact', () => {
    ...
    const header = { 'proxy-authorization': `Basic ${Buffer.from('run:a:b:c').toString('base64')}` };
...
test('the runner builds a PER-RUN proxy URL carrying the token, in env only', () => {
    ...
    assert.strictEqual(env.https_proxy, env.HTTPS_PROXY, 'lowercase twin: some clients only read that one');
    // NO_PROXY would be a hole in the only route out.
    assert.ok(!('NO_PROXY' in env) && !('no_proxy' in env), 'no exemption list may be set');
