...
test('the happy path: a scoped change reaches the feature branch at the remote', async () => {
    ...
    try {
        ...
        const pushed = git(['show', '--name-only', '--format=', 'refs/heads/feature/493/thing'], f.target);
...
test('the happy path survives repetition', async () => {
    ...
    try {
        ...
        for (let i = 1; i <= 3; i += 1) {
            ...
            assert.ok(refExists(f.target, `refs/heads/feature/493/run-${i}`), `run ${i} landed`);
        }
    } finally {
        await f.cleanup();
    }
    ...
    try {
        ...
        await assert.rejects(
            runStage({
                ...
                forgeUser: 'x-access-token',
                timeoutMs: 60000,
                push: { branch: 'main', issue: 493, scope: ['lib'], allowedRepo: f.targetUrl },
            }),
            (err) => err.code === 'protected_branch'
        );

        assert.strictEqual(mainSha(f.target), before, 'main is untouched at the remote');
    } finally {
        await f.cleanup();
    }
});
    ...
    try {
        ...
        await assert.rejects(
            runStage({
                ...
                forgeUser: 'x-access-token',
                timeoutMs: 60000,
                push: { branch: 'feature/493/x', issue: 493, scope: ['lib'], allowedRepo: f.targetUrl },
            }),
            (err) => err.code === 'repo_not_allowed'
        );

        assert.strictEqual(mainSha(f.other), before, 'the other repository is untouched');
        assert.ok(
    ...
    try {
        ...
        await assert.rejects(
            runStage({
                ...
                forgeUser: 'x-access-token',
                timeoutMs: 60000,
                push: { branch: 'feature/493/x', issue: 493, allowedRepo: f.targetUrl },
            }),
...
test('a change outside the declared scope is refused', async () => {
    ...
    try {
        ...
        assert.ok(refExists(f.target, 'refs/heads/feature/493/x') === false, 'nothing reached the remote');
    } finally {
        await f.cleanup();
    }
});
...
test('the command child does NOT hold the credential when the stage may push', async () => {
    ...
    const f = await fixture((root) => {
        ...
        return 'mkdir -p lib && echo x > lib/A.php\n'
    ...
    try {
        ...
        await runStage({
            ...
            forgeUser: 'x-access-token',
            timeoutMs: 60000,
            push: { branch: 'feature/493/env', issue: 493, scope: ['lib'], allowedRepo: f.targetUrl },
        });

        const seen = fs.readFileSync(dump, 'utf8');
        assert.match(seen, /GIT_FORGE_TOKEN=\[\]/, 'the token is absent from the child environment');
        assert.match(seen, /GIT_ASKPASS=\[\]/, 'the askpass helper is absent from the child environment');
        assert.ok(seen.includes(PASSWORD) === false, 'the token value appears nowhere in the child environment');
    } finally {
        await f.cleanup();
    }
});
...
test('an injected instruction cannot push: the child has no credential to push WITH', async () => {
    ...
    try {
        ...
        await runStage({
            ...
            forgeUser: 'x-access-token',
            timeoutMs: 60000,
            push: { branch: 'feature/493/inject', issue: 493, scope: ['lib'], allowedRepo: f.targetUrl },
        }).catch(() => { /* the stage's own push may also refuse; irrelevant here */ });

        const attempt = fs.readFileSync(log, 'utf8');
        assert.ok(attempt.includes('exit=0') === false, `the injected push should have failed, got: ${attempt}`);
        assert.strictEqual(mainSha(f.target), before, 'main is untouched at the remote');
    } finally {
        await f.cleanup();
    }
...
test('a stage that declares NO push keeps the credential in the command env', async () => {
    ...
    try {
        ...
        assert.match(fs.readFileSync(dump, 'utf8'), new RegExp(`TOKEN=\\[${PASSWORD}\\]`));
    } finally {
        await f.cleanup();
    }
});
...
test('nothing to push is reported as nothing pushed, not as a push', async () => {
    ...
    try {
        ...
        const result = await runStage({
            ...
            forgeUser: 'x-access-token',
            timeoutMs: 60000,
            push: { branch: 'feature/493/empty', issue: 493, allowedRepo: f.targetUrl },
        });
    ...
    try {
        ...
        try {
            ...
            await runStage({
                ...
                forgeUser: 'x-access-token',
                timeoutMs: 60000,
                push: { branch: 'feature/493/argv', issue: 493, scope: ['lib'], allowedRepo: f.targetUrl },
            });
        } finally {
            cp.spawn = realSpawn;
        }
        ...
        for (const line of seen) {
            assert.ok(line.includes(PASSWORD) === false, `credential found on argv: ${line}`);
        }
        assert.ok(seen.some((l) => l.includes('push')), 'a push actually happened');
    } finally {
        await f.cleanup();
    }
});
