...
function refuses(fn, code, what) {
	...
	assert.ok(
		thrown instanceof PushRefused,
		`${what}: expected PushRefused, got ${thrown.name}`,
	)
	assert.strictEqual(
		thrown.code,
		code,
		`${what}: wrong refusal code (message was: ${thrown.message})`,
	)
}
...
test('repoIdentity normalises the forms the same repository is written in', () => {
	...
	assert.strictEqual(
		repoIdentity('https://github.com/ConductionNL/hydra'),
		canonical,
	)
	assert.strictEqual(
		repoIdentity('https://github.com/ConductionNL/hydra.git'),
		canonical,
	)
	assert.strictEqual(
		repoIdentity('https://github.com/ConductionNL/hydra/'),
	...
	assert.strictEqual(
		repoIdentity('https://x-access-token@github.com/ConductionNL/hydra'),
		canonical,
	)
	assert.strictEqual(
		repoIdentity('https://GITHUB.COM/conductionnl/HYDRA'),
...
test('a fully-qualified ref is refused rather than normalised', () => {
	...
	refuses(
		() => assertBranchAllowed('refs/heads/main', 493),
		'qualified_ref',
		'refs/heads/main',
	)
	refuses(
		() => assertBranchAllowed('refs/tags/v1', 493),
		'qualified_ref',
		'a tag ref',
	)
})
...
test('a branch that merely CONTAINS the allowed pattern is refused', () => {
	...
	refuses(
		() => assertBranchAllowed('evil/feature/493/x', 493),
		'branch_not_allowed',
		'prefixed',
	)
	refuses(
		() => assertBranchAllowed('feature/493/x/../../main', 493),
		'branch_not_allowed',
		'traversal',
	)
	refuses(
		() => assertBranchAllowed('feature/4930/x', 493),
		'branch_not_allowed',
		'issue number prefix',
	)
	refuses(
		() => assertBranchAllowed('feature/493/x:refs/heads/main', 493),
		'branch_not_allowed',
		'refspec smuggling',
	)
})
...
test('a stage with no issue number cannot push anywhere', () => {
	...
	refuses(
		() => assertBranchAllowed('feature/493/x', ''),
		'issue_missing',
		'no issue',
	)
	refuses(
		() => assertBranchAllowed('feature/493/x', undefined),
		'issue_missing',
		'undefined issue',
	)
	refuses(
		() => assertBranchAllowed('feature/493/x', 'abc'),
		'issue_missing',
		'non-numeric issue',
	)
})
...
test('the allowed branch shape is accepted', () => {
	assert.doesNotThrow(() =>
		assertBranchAllowed('feature/493/builder-write-access', 493),
	)
...
test('a dependency manifest is refused at any depth', () => {
	refuses(
		() => assertDiffAllowed(['composer.json'], []),
		'dependency_manifest',
		'root composer.json',
	)
	refuses(
		() => assertDiffAllowed(['exapp/llm-runner/package.json'], []),
		'dependency_manifest',
		'nested package.json',
	)
	refuses(
		() => assertDiffAllowed(['package-lock.json'], []),
		'dependency_manifest',
		'a lockfile',
	)
	refuses(
		() => assertDiffAllowed(['src/go.mod'], []),
		'dependency_manifest',
		'go.mod',
	)
})
...
test('scope matches on directory boundaries, not string prefixes', () => {
	...
	assert.doesNotThrow(() =>
		assertDiffAllowed(['lib/Service/A.php'], ['lib/Service']),
	)
