...
test('the runner has no route out except the proxy', () => {
    ...
    assert.doesNotMatch(runner, /^\s+- egress\s*$/m, 'the runner is attached to the egress network');
    assert.match(compose, /jailed:\s*\n(\s+#[^\n]*\n)*\s+internal:\s*true/, '`jailed` is not internal');
...
test('both containers restart always — a silent Exited sidecar stops the pipeline', () => {
    assert.match(runner, /restart:\s*always/, 'runner restart policy is not `always`');
