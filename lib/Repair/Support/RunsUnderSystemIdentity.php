...
/**
 * Hermiq repair-step system-identity helper.
 *
 * @category Repair
 ...
 * dossiq) with a deliberately invalid payload, so nothing could be created
 ...
 *
 * @spec openspec/specs/admin-ui/spec.md
 */
trait RunsUnderSystemIdentity {
	 ...
	 *
	 * ⚠️ A TEST FAKE MUST INVOKE THE CALLABLE. `createMock()` stubs
	 * `runAsSystem()` to return null without running anything, which makes a
	 * CORRECT change look broken — the mirror of a fake that omits the method
	 * entirely and lets a broken change look green.
	 *
