<?php
/**
 * Hermiq DemoDataService.
 *
 * Imports `lib/Settings/hermiq_mock_register.json` — a `type: mock` descriptor
 * generated from this app's own schemas by
 * `hydra-gates/scripts/lib/generate_mock_register.py`, so every value is
 * conformant BY CONSTRUCTION rather than hand-written to look plausible.
 *
 * 🔴 ON DEMAND ONLY, NEVER ON INSTALL. A mock register has no Repair step and
 * is not part of `InitializeSettings`: demo data is something an operator asks
 * for from the setup wizard, and an install that silently seeds ninety example
 * objects into a production instance is a defect, not a convenience.
 *
 * @category Service
 ...
 *
 * @spec openspec/specs/configuration-initialization/spec.md
 */
...
class DemoDataService {
	 ...
	 * @var string
	 */
	private const DESCRIPTOR = '/lib/Settings/hermiq_mock_register.json';
	...
	public function install(): array {
		...
		// — an object whose schema does not resolve is SKIPPED, not errored.
		$objects = 0;
		$components = ($data['components'] ?? []);
	 ...
	 * instead of the RuntimeException above that names the missing app.
	 *
