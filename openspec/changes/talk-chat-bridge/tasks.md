...
- [x] 4.4 Ensure a failed turn never leaves the acknowledging reaction as the only signal, and prove both hand-offs route through the same `TalkTurnService` — divergence here means only one path is ever really exercised. Covered by `TalkTurnHandOffTest`.

  **Deliberately NOT implemented: the `TaskSuccessfulEvent` / `TaskFailedEvent` listener.** This task was written for a design in which the triggered path makes the turn a TaskProcessing *task*. 4.2 as built does not: `TalkTurnDispatcher` never calls `scheduleTask()`. It always enqueues `TalkTurnJob` as the durable carrier and uses `trigger()` purely as a nudge, so no TaskProcessing task exists for a Talk turn and those events could not fire for one. A listener for them would be unreachable code — the exact green-but-dead shape this change's §8 exists to catch — so it is recorded here rather than written.

  The concern behind the task is met on the path that does execute: `TalkTurnService::runTurn()` reports every failure into the originating room via `reportFailure()` — unresolvable speaker, empty answer, and any thrown error alike — so the reaction is never the only signal. Revisit if a triggerable Hermiq runner ever ships.
