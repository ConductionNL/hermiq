...
- [ ] 1.1 Add `tests/e2e/dashboard-and-agents.spec.ts` — log in, navigate to the Hermiq
...
- [ ] 1.2 Confirm `playwright.config.ts`'s `chromium` project (the default regression
      project) picks this file up — it must NOT be added to `testIgnore`.
- [ ] 1.3 Add stable `data-testid` attributes to the touched components if missing (use
      `/journeydoc-instrument` per ADR-030 if this repo already uses it) so selectors
      aren't brittle CSS-class matches.
- [ ] 1.4 Reference this spec from the relevant scenarios in `openspec/specs/dashboard-page/spec.md`
...
- [ ] 2.1 Add `tests/Unit/Controller/TenantOpsControllerTest.php` — cover `quota()` and
      `auditExport()` (happy path + at least one error path: unauthenticated/forbidden
      per ADR-008).
- [ ] 2.2 Add `tests/Unit/Controller/SkillMarketplaceControllerTest.php` — cover
      `installFromSource()`, `approve()`, `publish()` (happy path + rejection path for
      each).
- [ ] 2.3 Add `tests/Unit/Controller/MemoryControllerTest.php` — cover `memory()`,
      `addMemory()`, and at least one of `userProfiles()`/`sessions()`/`consolidate()`/`recall()`.

## 3. Verify

- [ ] 3.1 `composer phpunit` (unit suite) — new tests pass.
- [ ] 3.2 `npx playwright test --project chromium` against a running dev instance — the
      new spec passes and the project is no longer empty.
- [ ] 3.3 `composer check:strict`.
