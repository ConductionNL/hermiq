...
Hermiq's Agent detail and list pages render as manifest-driven pages through the nc-vue
manifest-v2 renderer (`type:"detail"` / `type:"index"`) instead of bespoke `type:"custom"`
Vue components, matching the fleet convention (ADR-036 universal widget manifest, ADR-049
keep-custom-near-zero, ADR-062 widget-grid discipline). Genuinely bespoke interactions
(dry-run/replay, run-trace observability, tool-catalogue editing, quarantine approval)
remain small, individually justified `type:"custom"` escape-hatch widgets rather than
opaque full pages.
...
- **Performance:** The agent-detail widget grid MUST NOT issue more network requests than the
  original `AgentDetail.vue` on initial load (the same six fetches — agent, schema, schedule,
  runs, tools, skills — are redistributed across widgets, not duplicated per widget).
- **Accessibility:** Every `NcSelect` in the extracted widgets MUST carry an `inputLabel`
  (unchanged from the current implementation) — WCAG 2.1 AA (ADR-004).
- **Internationalization:** Dutch and English MUST be supported for every new/changed
  user-facing string (ADR-005) via `l10n/en.json` + `l10n/nl.json`.

## Acceptance Criteria

- [ ] `/agents/:id` renders as `type:"detail"` with the six-widget grid described in REQ-002
  through REQ-007, and no widget exceeds its declared `gridHeight` (no inner scroll).
- [ ] `/agents`, `/agent-templates`, `/evals` render as `type:"index"`.
- [ ] `/evals/:id` (`EvalDatasetDetail`) exists and hosts the `eval-run-panel` widget.
- [ ] `AgentFormModal` is registered once and reused by both REQ-008 and REQ-009.
- [ ] "Approve" on `AgentTemplateGallery` still calls the guarded `approveAgentTemplate`
  endpoint (verified: no `object-op` patch on `agenttemplate.state`).
- [ ] `npm run check:specs`, `npm run lint`, `npm test` all pass.
- [ ] `tests/e2e/dashboard-and-agents.spec.ts` and `tests/e2e/wave2-surfaces.spec.ts` pass
  against the converted pages.

## Notes

- Deferred: `Chat`, `ApprovalInbox`, `AgentMemory`, `AgentSessions`, `SkillsCatalog`,
  `AiFeatureRegister`, `TenantOps`, `McpTools`, `ComplianceDashboard` stay `type:"custom"` —
  each already carries a spec-reviewed `_note` justification at HEAD; out of scope here.
- Deferred: a reverse-FK OpenRegister calculation primitive that would let list pages show
  derived cross-collection columns (schedule-attached, last-run, latest-pass-rate) without a
  bespoke join — see design.md Decision 9.
- Related ADRs: ADR-036 (universal widget manifest), ADR-049 (keep-custom-near-zero),
  ADR-062 (widget-grid fill discipline), ADR-023 (action authorization), ADR-004 (modal
  isolation, WCAG), ADR-005 (i18n).
