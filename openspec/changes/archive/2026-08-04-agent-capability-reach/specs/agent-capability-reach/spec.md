# agent-capability-reach (delta)

A second, orthogonal risk axis for agent tools. `scope` says which data verb a tool performs;
**`reach`** says how far a successful invocation travels — `self` < `user` < `instance` < `external`.
Default-deny and the human-approval gate key off reach in union with today's write/destructive rule,
so the tools that leave the building stop being classified by their CRUD verb alone. A per-grant
`#noapproval` waiver lets an owner run one already-narrowed capability unattended, audibly and
reversibly.
...
- **Performance:** reach resolution MUST be a pure in-memory lookup over the descriptor already
  fetched for classification, adding no additional catalogue round-trip per turn assembly.
- **Accessibility:** no new user-facing surface ships in this change; the reach and waiver UI
  obligations (WCAG 2.2 AA, NC components, no hardcoded colours, modals under `src/modals/`) belong
  to `grant-waiver-ui`.
- **Internationalization:** any new user-facing string added on the audit or refusal path MUST carry
  Dutch (`nl_NL`) and English (`en_US`) translations (ADR-005).

## Acceptance Criteria

- [ ] A closed, ordered `reach` vocabulary exists and is orthogonal to `scope`
- [ ] All 14 native `hermiq.*` descriptors declare a reach matching design.md's table
- [ ] An absent or unrecognised `reach` resolves to `external`, never to `self`
- [ ] Derived `{app}.{schema}.{verb}` ids infer `user` for read verbs and `instance` for write verbs,
      and `external` for anything else
- [ ] Gating is the union of the existing write/destructive rule and `reach >= instance`; no tool
      becomes more permissive
- [ ] The approval gate receives the catalogue descriptor, closing the hint bypass on that path
- [ ] `hermiq.webSearch` and `hermiq.webFetch` are default-denied and gated unless explicitly granted
- [ ] A delegation's effective reach is the maximum of the tool's reach and the target's granted reach
- [ ] `#noapproval` parses as a fragment split off before the `?` split, and every pre-existing grant
      string parses identically
- [ ] The waiver suppresses the approval gate only — it widens no grant, relaxes no constraint and
      bypasses no RBAC
- [ ] Every path that persists a grant list verifies the acting user is the agent's owner server-side
- [ ] Adding or removing a waiver writes a distinct, greppable audit event
- [ ] `docs/tool-grants.md` covers the full grammar, the reach vocabulary with a worked example per
      level, default-deny, when the gate fires, and what waiving gives up

## Notes

- Depends on `agent-object-owner-authorization` (`kind: config`) for the owner-scoped Agent schema
  authorization block. Without it the generic OpenRegister object write path stays default-open and
  the owner-only requirement cannot hold. The bypass is REPRODUCED, not inferred — a non-owner PUT
  returned HTTP 200 and rewrote an admin-owned agent's `tools` to include `hermiq.sendMail`. See
  design.md Decision 9 for the method and result, and "Fleet-wide finding" for the
  `enforce_default_closed` default that reaches beyond hermiq.
- `mail-app-capability-ladder` and `grant-waiver-ui` complete the chain; the mail rungs and the
  editor rewrite are deliberately out of this change.
- Related: **ADR-023** (data RBAC is OpenRegister's, action RBAC is the app's), **ADR-004**
  (governance via OpenRegister's AuditTrail), **ADR-031** (declarative over service classes),
  **ADR-032** (spec sizing and chaining), **ADR-035 D4** (`Agent.tools` is a frozen `string[]`),
  **ADR-063** (the derived MCP catalogue this consumes).
- Capabilities this change touches: `agent-tool-governance` (grant grammar and default-deny),
  `human-approval-gate` (when the gate fires), `nc-native-tools` (the descriptors that gain a reach),
  `web-research-tool` (the two tools whose classification changes).
