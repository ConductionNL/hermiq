...
Establishes a real (non-skeleton) Playwright regression project and closes the highest-risk
PHPUnit coverage gaps, replacing the current state where the default regression project
runs zero tests and several state-mutating controllers have no test coverage at all.
