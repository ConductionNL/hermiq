...
**Scope**: hermiq
**OpenSpec changes**:
- `openspec/changes/sub-agent-delegation/` — the `hermiq.delegateAgent` tool, `DelegationService`,
  `DelegationContext`, and the `Agent.delegationAllowlist` schema field (kind: code)

## Purpose
Lets one Hermiq agent invoke another agent — in the same organisation, on its own explicit allowlist —
as a bounded, sequential sub-task with an isolated conversation, while inheriting the SAME governance
rails (kill-switch, budget hard-cap, tenant-model-policy, acting-user attribution, redacted audit
trail) every other governed run in Hermiq already gets. Closes the most common "advanced" capability
gap across the competitive field (sub-agent delegation / handoffs) without introducing a second,
weaker enforcement path alongside the existing one.
...
- **Performance:** the delegation gate checks (allowlist/cycle/depth/fan-out/organisation/model-policy)
  MUST add no register/schema-wide query beyond what `ScheduleService`'s existing kill-switch/budget
  reads already perform once per invocation.
- **Accessibility:** the `delegationAllowlist` editor in `AgentFormModal.vue` MUST meet WCAG 2.1 AA,
  reusing the existing `tools` field's `NcSelect`/`inputLabel` pattern.
- **Internationalization:** Dutch and English MUST be supported for every new user-facing string
  (ADR-005).

## Acceptance Criteria

- [ ] An agent with an empty `delegationAllowlist` cannot delegate to any other agent.
- [ ] Self-delegation and any cyclic delegation chain are refused regardless of allowlist content.
- [ ] Delegation depth and fan-out are enforced from server-side state, never from tool-call arguments.
- [ ] Cross-organisation delegation is refused unconditionally.
- [ ] A delegated sub-run is impersonated as the parent's acting user, never the target agent's own.
- [ ] A delegated sub-run cannot see the parent conversation's prior history.
- [ ] Kill-switch, budget hard-cap, and approval-required targets all refuse delegation before any
      sub-agent invocation.
- [ ] A successful delegated run's `AuditTrail` entry carries a `runId` and `parentRunId` correlating
      it to the calling run.

## Notes
- Parallel/concurrent sub-agent execution, shared mutable memory between agents, and free-form
  agent-to-agent chat are explicitly out of scope for this capability — see the change proposal's
  Out of Scope section.
- Depends on `multi-tenant-ops` (budget hard-cap, kill-switch, tenant isolation), `tenant-model-policy`
  (per-organisation provider/model allowlist), `agent-engine-port` (the in-app `Engine`/`ToolLoop` this
  tool only runs on), and `run-audit-log`/`observability` (the audit write-path and trace timeline it
  reuses).
