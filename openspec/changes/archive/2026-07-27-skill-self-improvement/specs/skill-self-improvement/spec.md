...
Closes the ADR-068 §5 loop for hermiq product skills: a background consolidation job
proposes DRAFT new skill versions from accumulated `files['learnings.md']` entries, every
draft passes a pre-qualification pipeline (content scan + paired draft-vs-active eval)
before a human sees it, acceptance runs through the human-approval-gate `Approval` state
machine behind ADR-023 action authorization, skills gain agent-versioning-style
history/diff/rollback/run-pinning, and an accepted version that postdates a GitHub
publish raises an explicit, never-automatic republish signal. The gated path is
non-negotiable: draft → approval → re-scan → eval regression gate → versioned rollback —
...
- **Performance:** the consolidation job processes skills sequentially with per-skill
  try/catch (one failure never aborts the pass or the fleet cron); the catalog
  behind-badge is computed client-side from `publishedAt` vs `lastAcceptedVersionAt`
  (no per-row history query); version diff for a 500-line body + 20 files MUST return
  in under 2 seconds.
- **Accessibility:** WCAG 2.2 AA; diff added/removed and pass/fail never color-only
  (1.4.1); review actions and badges carry accessible names (1.3.1, 4.1.2); target size
  met via standard NC components (2.5.8).
- **Internationalization:** Dutch and English MUST be supported (ADR-007) for all
  review-surface, badge, and notification strings.

## Acceptance Criteria

- [ ] `agentskilldraft` schema + `Skill.lastAcceptedVersionAt` land via forced re-import; existing objects unaffected
- [ ] Consolidation drafts on threshold/regression/manual triggers; one open draft per skill; active skill never edited in place
- [ ] Kill-switch + budget hard-caps gate the LLM pass and the paired eval exactly as schedule ticks
- [ ] Content scan covers `learnings.md` as instruction content; dangerous → discard, no override
- [ ] Paired eval auto-discards a strictly-worse draft (learnings retained); no evals → `noEvalEvidence` flag, never L5
- [ ] Accept/Edit-then-accept/Reject behind `skill.review-draft`, through the Approval state machine; approving the Approval from ANY surface (generic inbox included) applies the draft; the Approval payload carries deep link + scan verdict + eval delta + learnings summary or is invalid; draft edits re-run pre-qualification before the Approval is approvable
- [ ] Version history/diff/rollback over `frontmatter`/`body`/`files`; history never mutated; runs pin skill versions (never fatal)
- [ ] Post-acceptance regression → rollback suggestion + notification (advisory only)
- [ ] Behind-badge + notification + authorized one-click Republish; never automatic
- [ ] Every transition/rollback/republish audited via OR AuditTrail
- [ ] Seeded pending draft renders a decidable review surface (Playwright)

## Notes

- ADR-068 §5 is the governing decision; the "Prompt-injection amplification" table in
  design.md restates why every gate exists — gates are spec, not tuning.
- ADR-023 governs `skill.review-draft` and the reused publish authorization; ADR-031
  justification for the imperative pipeline is design.md Decision 1; ADR-060 requires
  the eval evidence to come from real executed runs.
- The `learning-candidates.md` export strip and the republish carve-out to the
  provenance repo are specified as a delta on the skills-marketplace publish
  requirement (see `specs/skills-marketplace/spec.md` in this change).
- Learnings capture/promotion/redaction and `levelEvidence.l6` are owned by
  `skill-learnings`; the paired eval mode and `levelEvidence.l5` by `skill-evals`;
  this capability consumes both by name.
