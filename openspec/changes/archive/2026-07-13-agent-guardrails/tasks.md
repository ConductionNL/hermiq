...
- [x] Implement
- [x] Test (compile/structural-verified: `GuardrailPolicy` schema + `Approval.sourceType=toolcall`/`toolId`/`toolArguments`/`consumedAt` fields present and pass `npm run check:register`/`check:json-strict`; info.xml diffed 0.1.52 → 0.1.53)
...
- [x] Implement
- [x] Test (`tests/Unit/Service/GuardrailPolicyServiceTest.php` — resolution order org→instance→fully-open fallback, disabled-policy fallthrough, filterInput/filterOutput redact/block/off, classifyTool auto/confirm/deny, upsertForOrganisation validation)
...
- [x] Implement
- [x] Test (`tests/Unit/Service/Engine/EngineTest.php::testProcessMessageThrowsAndPersistsNothingWhenInputIsBlockedByGuardrailPolicy` + `::testProcessMessageRedactsInputAndOutputViaGuardrailPolicy`, added during this pass — previously this wiring had zero unit coverage)
...
- [x] Implement
- [x] Test (`tests/Unit/Service/ScheduleServiceTest.php` guardrail-block/redact cases, plus `FlowAgentRunServiceTest.php`/`WebhookAgentRunServiceTest.php` unaffected — confirmed no changes needed in those two)
...
- [x] Implement
- [x] Test (`tests/Unit/Service/Engine/FacadeToolInvokerTest.php` + `tests/Unit/Service/Engine/ToolLoopTest.php`)
...
- [x] Implement
- [x] Test (`tests/Unit/Service/ApprovalServiceTest.php`)
...
- [x] Implement
- [x] Test (`tests/Unit/Service/Engine/FacadeToolInvokerTest.php` + `tests/Unit/Service/ApprovalServiceTest.php`)
...
- [x] Implement (`lib/Controller/GuardrailPolicyController.php` mirrors `TenantModelPolicyController`'s effective/index/create/update split and `mayAdminister()` gating exactly; routes registered in `appinfo/routes.php` with `effective` before `{policyId}`; `src/api/guardrailPolicy.js` mirrors `modelPolicy.js`; `TenantOps.vue` gained a "Guardrail policy" section mirroring the "Model policy" section's inline-editor shape. NOTE: design.md's API Design section describes a single query-param `PUT .../guardrail-policies?organisation=<id>` upsert; this app's ACTUAL established pattern — and `GuardrailPolicyService`'s own `upsertForOrganisation()`/`update(uuid)` split — is separate POST-create + PUT-by-id-update, matching `TenantModelPolicyController`/`tenant-model-policy` 1:1, so the real routes/controller follow that precedent instead. File named `guardrailPolicy.js` per design.md's File Structure section and tasks.md's own `files` list, not the build-brief's `guardrails.js`.)
- [x] Test (compile-verified via phpcs/phpstan/phpunit-full-suite + `npm run lint`/`check:specs`; live browser coverage of the admin UI deferred to the playwright-regression-coverage change)
...
- [x] Implement (added the blocked-content chat error to `ChatController::sendMessage()` — a `422`/`errorCode: guardrail_blocked` case, matching `GuardrailBlockedException`'s own docblock intent that a stable errorCode should let the frontend key off it — plus every new "Guardrail policy" admin-form label/action string; the tool-confirm approval notification reuses the EXISTING `approval_requested` wording (`Notifier::approvalRequestedText()`), so no new key was needed there)
- [x] Test (`python3 -c "import json; json.load(...)"` valid-JSON check on both files; every new key present in both with an English/Dutch value — verified by direct inspection)
