# agent-lifecycle-governance (delta)

Governs the lifecycle of the **human** behind an agent, not just the agent's individual runs:
automatic offboarding when a Nextcloud user is deleted or disabled, a periodic org-admin access
review of the standing agent inventory with an auditable attestation, and durable incident records
linking human-authored incident response to the runs/agents involved. Complements the existing
per-run governance (`run-audit-log`, `human-approval-gate`, `multi-tenant-ops`) to cover EU AI Act
Art. 12 (record-keeping across the system's lifecycle) and Art. 14 (human oversight, including
when the overseeing human is no longer present) end to end.
...
- **Performance:** The access review list and incident list MUST render within the same
  time budget as the existing quota/audit-export reads on `TenantOps.vue` (no additional
  cross-tenant scan; both are tenant-scoped `ObjectService` reads, same as `loadObjects()`).
- **Accessibility:** New `TenantOps.vue` sections MUST meet WCAG AA (reuse `NcButton`/
  `NcNoteCard`/`CnDataTable` accessible defaults; no hand-rolled interactive controls).
- **Internationalization:** Dutch and English MUST be supported (ADR-005) for all new UI strings.

## Acceptance Criteria

- [ ] Deleting or disabling a Nextcloud user pauses (disables) every Schedule owned by, or whose
  Agent's `actingUser` resolves to, that user, without waiting for the next scheduled tick.
- [ ] Every Agent affected by an offboarding pause is flagged for reassignment.
- [ ] An org admin can reassign a flagged Agent to a new active user, which clears the flag but
  does not auto-re-enable any paused Schedule.
- [ ] An org admin can list the organisation's Agents with owner, `actingUser`, last-run timestamp,
  and a tool/RAG capability summary.
- [ ] An org admin can attest that an Agent has been reviewed; the attestation records reviewer +
  timestamp and is durable/auditable.
- [ ] An org admin can open an incident record (description/impact/actions) linked to a run and/or
  Agent; incidents are tenant-scoped and listed newest first.
- [ ] The EU AI Act audit export includes incident records alongside existing run/approval entries.

## Notes

- Builds directly on `ScheduleService`'s existing `enabled`-flip + audit-write pattern (the same
  mechanic `recordGateSkip()` already uses for kill-switch/approval-gate skips) — no new pause
  primitive.
- The `Incident` schema is declarative (`lib/Settings/hermiq_register.json`), inheriting
  owner/organisation scoping from `ObjectEntity` and an `AuditTrail` entry automatically (ADR-004
  single write-path) — no bespoke incident audit code.
- Related: **ADR-004** (governance via OR AuditTrail), `run-audit-log` (per-run record-keeping),
  `human-approval-gate` (per-run oversight), `multi-tenant-ops` (org-level controls this change
  extends the UI home of), `ai-feature-governance-register` (design-time risk register this
  change's lifecycle governance complements — high-risk features still need the runtime controls
  covered here).
- Bulk/mass offboarding (e.g. a CSV of leavers) and automated retention-boundary purge are
  explicitly out of scope (see proposal.md).
