...
- [x] 1.1 Port `ChatService` (455 lines) + `Chat/*` handlers (`ResponseGenerationHandler`,
...
- [x] 1.2 Re-point all reads/writes at `Agent`/`Conversation`/`Message`/`Feedback` OR objects
  (`agent-engine-schemas`) via `ObjectService`, replacing OR's QBMapper entity/mapper calls.
- [x] 1.3 `ContextRetrievalHandler`'s RAG calls remain a data query against OR's existing vector-
  search surface — do not duplicate `VectorEmbeddings`/`VectorSearchHandler` inside Hermiq.

## 2. Port the LLM provider layer

- [x] 2.1 Port the LLPhant provider layer + `llphant-ollama-think-keepalive.patch` +
...
- [x] 2.2 Add the 4th `nextcloud` driver: `OCP\TaskProcessing\IManager`-backed, non-streaming,
...
- [x] 3.1 Port `ToolManagementHandler` into `lib/Service/Engine/ToolLoop`, consuming
...
- [x] 3.2 Enforce the `Agent.tools` whitelist (empty = allow all) when calling
  `listTools()`, per the ADR-035 semantics `agent-engine-schemas` declared.

## 4. Mirror the routes

- [x] 4.1 Mirror OR's ~30 routes across `Chat`/`ChatStream`/`ChatHealth`/`Conversation`/`Agents`
...
- [x] 4.2 Preserve the SSE six-event envelope exactly (`token`/`tool_call`/`tool_result`/
...
- [x] 5.1 Merge OR's chat/agents SPA pages into Hermiq's existing SPA (already has
  `AgentCatalog.vue`/`AgentDetail.vue`/`AgentFormModal.vue`; gains Chat).
- [x] 5.2 Rewrite `src/api/agents.js` onto `createObjectStore` against
...
- [x] 6.1 Add an `IAppConfig` feature flag (e.g. `hermiq.engine.enabled`, default `false`) gating
  which engine `ScheduleService::runAgentAsOwner()` calls.
- [x] 6.2 When the flag is on, `runAgentAsOwner()` calls the new in-app `Engine` facade against a
...
- [x] 7.1 Build and run the parity harness (plan §7.5): same `(agent, prompt)` through both engine
...
- [x] 8.1 Re-run the full kill-switch/approval-gate regression suite (95 tests, from
...
- [x] 8.2 NC34 compat check on every new file: `\OCP\Server::get()` only (no `\OC::$server->
  getXxx()`); no second `<background-jobs>` block introduced in `info.xml`.

## 9. Quality gates

- [x] 9.1 Run the full Hydra mechanical gate suite (spdx-headers, forbidden-patterns, stub-scan,
...
- [x] 9.2 Composer audit + PHPCS/PHPMD/Psalm/PHPStan (`composer check:strict`) clean on all new
  `Engine`/`Llm` classes.
