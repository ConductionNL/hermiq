...
**OpenSpec changes:** `hermiq-agent-leaf` — DONE (21 of 21 tasks): the integration leaf that puts an
Agent tab on any object in any fleet app, the object-scoped chat that reuses the tool-free surface,
per-object run history, the run-on-object endpoint scoped to the caller's object permissions, the
declarative bounded-context allowlist, and the manifest agent action-type contract.

⚠️ A cross-app leaf ships in two halves and only one of them is checked. Gate 24 passes on the PHP
half and on the JS half independently, so a leaf whose JS is never loaded renders nothing and
reports green. Verify the leaf renders on a host app, not that both halves exist.

## Purpose

Hermiq agents where the work is. A user looking at a case in another app should be able to ask an
agent about that case, and run one on it, without leaving the object.

The leaf is bounded on purpose: it chats through the tool-free assistant surface, and running on the
object is scoped to the permissions the caller already has on that object. An agent reachable from
everywhere must not be able to do more from there than the person who invoked it.
