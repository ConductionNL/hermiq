...
**Status**: partial (schema shipped, assembler and editor open)
...
re-import. `hermiq-context-documents` — OPEN: the run-start assembler that renders a document into
the context preamble, and the authoring surface that edits one, are not built. A Context object can
therefore hold documents that nothing yet reads.
