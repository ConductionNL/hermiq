# Agent Context System Specification

**Status**: active (inline `documents` source kind is specified separately, see context-documents)

**Feature tier**: V1

**OpenSpec changes:** `agent-context-system` — DONE (14 of 14 tasks): the `Context` object, its
char-budget consolidation flag, run-start assembly of object queries and file refs, and the
`Agent.contextRefs` attachment that prepends the assembled text to the system prompt.

## Purpose

A named, reusable bundle of grounding material an agent carries into every run. Implements
`SPECTR-NEXTCLOUD-PLAN.md` section 6.4.

Context is the durable half of what an agent knows. RAG answers the turn; a Context answers the job,
and it is prepended ahead of the turn's RAG block so per-turn retrieval refines it rather than
competing with it.
