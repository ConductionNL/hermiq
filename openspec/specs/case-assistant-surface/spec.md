# Case Assistant Surface Specification

**Status**: active

**Feature tier**: V1

**OpenSpec changes:** `case-assistant-surface` — DONE (11 of 11 tasks): `POST /api/assistant/converse`,
a synchronous single-request grounded chat with no tool execution, ownership-scoped session reuse,
input caps, and the same guardrail and audit path as `chat#sendMessage`.

## Purpose

A small conversational endpoint for a leaf app that wants grounded chat about the object on screen,
and nothing else. No tools, no RAG orchestration, one request and one reply.

The tool-free property is the feature, not a limitation. A leaf app embedding a chat box on a case
does not want that box able to act on the case, and the cheapest way to guarantee that is a surface
that cannot.
