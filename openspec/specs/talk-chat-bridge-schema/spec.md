# Talk Chat Bridge Schema Specification

**Status**: done
**Standards**: OpenAPI 3.0.0 schema register (`lib/Settings/hermiq_register.json`)
**Feature tier**: MVP

**OpenSpec changes:**
- `openspec/changes/archive/2026-07-28-talk-chat-bridge-schema/` — the four properties and the Agent opt-in flag (kind: config) — **done**

## Purpose

The declarative data shape the Talk chat bridge stands on: a filterable room↔session binding, a
participant roster, per-turn human authorship, and the Hermiq half of the two-sided opt-in.

Every property is OPTIONAL and none is added to a `required` list, so the shape landed inert —
existing conversations, messages and agents stayed valid with no backfill and no behaviour change
until `talk-chat-bridge` shipped the code that reads them.

The one decision worth carrying forward: `talkRoomToken` is declared **top-level rather than
inside `metadata`** because it is a key the bridge must RESOLVE BY. Measured live on the target
instance: a top-level filter returns the bound conversation, while the same value under
`metadata.talkRoomToken` returns zero rows — silently, with no error. Nested placement would have
produced a bridge that looked wired and was dead, with every unit test still green.
