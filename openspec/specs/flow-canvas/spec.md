...
### Requirement: A node card names the STEP it runs @e2e tests/e2e/flow-builder-dialect.spec.ts
...
### Requirement: A connection shows only its own title @e2e tests/e2e/flow-builder-dialect.spec.ts
...
### Requirement: Role is expressed by the ABSENCE of a port @e2e tests/e2e/flow-builder-dialect.spec.ts
...
### Requirement: A routing node exposes one NAMED out-port per branch @e2e tests/e2e/flow-builder-dialect.spec.ts
...
### Requirement: Saving warns about a dead end; it never refuses @e2e exclude requires a save round-trip against a live engine — covered by the OpenRegister side
...
### Requirement: The flow list shows last run and status @e2e exclude list rendering over API fields — covered by the OpenRegister side
