# Talk Approval Reactions Schema Specification

**Status**: active

**Feature tier**: V1

**OpenSpec changes:** `talk-approval-reactions-schema` — DONE (9 of 9 tasks): the `Approval` schema
records the Talk message that carries it and how it was decided, added union-import-safe so the
re-import actually applies it.

## Purpose

The schema half of approving an agent action from a Talk room. An approval decided by a reaction has
to remember which message it reacted to, or the decision cannot be traced back to the person who
made it.
