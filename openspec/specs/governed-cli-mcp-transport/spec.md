...
**OpenSpec changes:** `cli-runner-governed-mcp-and-egress` — DONE (16 of 16 tasks): Hermiq serves a
run-scoped MCP endpoint whose `tools/list` is the agent's resolved grant set and whose `tools/call`
dispatches through `FacadeToolInvoker`, so guardrails, the approval gate, redaction, model policy,
budgets and run tracing all still apply on the `cli` path. The run-scoped token, the CLI invocation
flags that lock the transport, and the single allowed-URL egress policy ship with it.
