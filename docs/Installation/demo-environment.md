...
This page gets a working Hermiq running on your own machine in two commands. You end with agents and approvals.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/hermiq/development/hermiq-compose.yaml
```
...
==> installing hermiq <version>
==> apps present: hermiq integriq openregister thematiq
```
...
```bash
curl -s http://localhost:8617/status.php
```
...
| What | Where |
| --- | --- |
| **Hermiq** | [http://localhost:8617/apps/hermiq/](http://localhost:8617/apps/hermiq/) |
| Admin interface | [http://localhost:8617](http://localhost:8617) — `admin` / `admin` |
...
| `hermiq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing hermiq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -o /dev/null -w '%{http_code}\n' -u admin:admin -L "http://localhost:8617/apps/hermiq/"
...
curl -s -u admin:admin "http://localhost:8617/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f hermiq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f hermiq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f hermiq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
